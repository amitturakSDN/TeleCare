...
import {styles} from '@/screens/TakeReadings/Manual/BloodGlucose/BloodGlucose.styles';
...
export function BloodGlucose({navigation}) {
  ...
  const addReadings = () => {
    if (Validate.empty(glucoseValue)) {
      return Validate.errorDisplay(strings.validation.empty, setReadingsError);
    }  
    else if(Validate.single_decimal(glucoseValue)) {
      return Validate.errorDisplay(strings.validation.invalidSingleDecimal, setReadingsError);
    }
    else {
      if(glucoseValue.length > 3 && Validate.force_single_decimal(glucoseValue)) {
      ...
      <View style={{margin: moderateScale(20)}}>
        <View
          ...
          <View
            ...
            <TextField1
              title={strings.bloodGlucose.subTitle}
              maxLength={5}
              autoFocus={true}
              //  placeholder={'98.3'}
              ...
              scale={true}
              scaleName={unit}
              // scaleName={strings.bloodGlucose.scale}
            />
          </View>
          <View
            style={{
