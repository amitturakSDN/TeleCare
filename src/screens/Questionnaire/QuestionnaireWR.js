...
export function QuestionnaireWR({navigation, route}) {
    const dispatch = useDispatch();
    let {type} = route.params;
    const {questionnaireDataWR} = useSelector(state => state.appointment);
    ...
    useEffect(() => {
        setQuestionListData();
        // console.log('Enterrrrr', JSON.stringify(questionnaireDataWR, undefined, 4));
    }, []);

    useEffect(() => {}, [questionnaireDataWR]);

    /**Set array for questionire list */
    const setQuestionListData = () => {
        if (questionnaireDataWR?.getCareQuestionnaireWR) {
            let all_question = [...questionnaireDataWR?.getCareQuestionnaireWR[0]?.item];
    ...
    const validateQuestion = () => {
        let updated_question = questionList.map(m => {
            if (
                m?.answerType === 'freeText' ||
                m?.answerType == 'dropDown' ||
                m?.answerType == 'string'
            ) {
                m.error =
        ...
        if (questionList.filter(m => m.answer.toString().trim() == '' && m.required).length == 0) {
            ...
            questionList.map(subQuestion => {
                if (
                    subQuestion?.answerType === 'freeText' ||
                    subQuestion?.answerType === 'string' ||
                    subQuestion?.answerType == 'dropDown'
                ) {
    ...
    const saveQuestion = questionResponse => {
        let param = {
            ...
            questionResponse,
            questionnaireId: questionnaireDataWR.getCareQuestionnaireWR[0].id,
        };
        dispatch(
        ...
        type == 'booking'
            ? navigation.push(NAVIGATION.getCareForm, {location: location}, {location: location, description: reason, orgId: orgId})
    ...
    const renderQuestion = ({item, index}) => {
        return (
            <View key={index} style={{flex: 1}}>
                {(item?.answerType == 'freeText' || item?.answerType == 'string') && (
                    <>
                        <TextArea
                            title={`${item.text}`}
